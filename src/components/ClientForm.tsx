        ...
        <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            <div className="mb-8">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button type="button" onClick={agregarDireccion} className="text-sm bg-gray-100 text-gray-700 px-3 py-1 rounded">+ Añadir Dirección</button>
                </div>
                {direcciones.map((dir, index) => (
                    <div key={index} className="flex gap-4 mb-3 items-start">
                        <input type="text" placeholder="Dirección completa" required value={dir.direccion} onChange={(e) => actualizarDireccion(index, 'direccion', e.target.value)} className="flex-1 px-3 py-2 border rounded-md text-black" />
                        <input type="text" placeholder="Ciudad" required value={dir.ciudad} onChange={(e) => actualizarDireccion(index, 'ciudad', e.target.value)} className="flex-1 px-3 py-2 border rounded-md text-black" />
                        {direcciones.length > 1 && <button type="button" onClick={() => removerDireccion(index)} className="px-3 py-2 text-red-600">X</button>}
                    </div>
                ))}
            </div>
            ...
            <div className="mb-8">
                <div className="flex justify-between items-center mb-4">
                    ...
                    <button type="button" onClick={agregarDocumento} className="text-sm bg-gray-100 text-gray-700 px-3 py-1 rounded">+ Añadir Documento</button>
                </div>
                {documentos.map((doc, index) => (
                    <div key={index} className="flex gap-4 mb-3 items-start">
                        <select value={doc.tipoDocumento} onChange={(e) => actualizarDocumento(index, 'tipoDocumento', e.target.value)} className="w-1/3 px-3 py-2 border rounded-md text-black bg-white">
                            <option value="DUI">DUI</option>
                            <option value="NIT">NIT</option>
                            <option value="Pasaporte">Pasaporte</option>
                        </select>
                        <input type="text" placeholder="Número" required value={doc.numero} onChange={(e) => actualizarDocumento(index, 'numero', e.target.value)} className="flex-1 px-3 py-2 border rounded-md text-black" />
                        {documentos.length > 1 && <button type="button" onClick={() => removerDocumento(index)} className="px-3 py-2 text-red-600">X</button>}
                    </div>
                ))}
            </div>

            <div className="flex justify-end gap-4 mt-8">
                <button type="button" onClick={onCancel} className="px-6 py-2 border text-gray-700 rounded-md">Cancelar</button>
                <button type="submit" disabled={isSubmitting} className="px-6 py-2 bg-blue-600 text-white rounded-md disabled:opacity-50">
